...
/**
 * Unit test case for the GnuCashImporter class
 * 
 * @author Kyle Treubig <kyle@vimofthevine.com>
 */
public class GnuCashImporterTest {
	
	/**
	 * The GnuCash importer
	 */
	private GnuCashImporter importer;
	...
	public void setUp()
	{
		try
		{
			progress = new TaskProgress("GnuCash import");
			...
			importer = new GnuCashImporter(currencyFactory);
	...
	{		
		try
        {
			Transaction[] expected = new Transaction[] {
				new ImportedTransaction(new SimpleDate(5, 10, 2010), "Payday",
					"", currencyFactory.newCurrencyInstance("150.0"),
					new Account("Income:Salary"), new Account("Expenses:Taxes:Federal")),
				new ImportedTransaction(new SimpleDate(5, 10, 2010), "Payday",
					"", currencyFactory.newCurrencyInstance("67.25"),
					new Account("Income:Salary"), new Account("Expenses:Taxes:State")),
				new ImportedTransaction(new SimpleDate(5, 10, 2010), "Payday",
					"", currencyFactory.newCurrencyInstance("29.76"),
					new Account("Income:Salary"), new Account("Expenses:Insurance:Health Insurance")),
				new ImportedTransaction(new SimpleDate(5, 10, 2010), "Payday",
					"Earned", currencyFactory.newCurrencyInstance("650.03"),
					new Account("Income:Salary"), new Account("Assets:Current Assets:Checking Account")),
				new ImportedTransaction(new SimpleDate(7, 11, 2011), "Mr. Mechanic",
					"Tires", currencyFactory.newCurrencyInstance("402.44"),
					new Account("Assets:Current Assets:Checking Account"), new Account("Expenses:Auto:Repair and Maintenance")),
				new ImportedTransaction(new SimpleDate(28, 6, 2010), "Vendor",
					"Entertainment", currencyFactory.newCurrencyInstance("150.0"),
					new Account("Assets:Current Assets:Checking Account"), new Account("Expenses:Entertainment:Recreation")),
				new ImportedTransaction(new SimpleDate(28, 6, 2010), "Vendor",
					"Food", currencyFactory.newCurrencyInstance("14.68"),
					new Account("Assets:Current Assets:Checking Account"), new Account("Expenses:Entertainment:Dining")),
			};
			...
			for (int i=0; i<transactions.size(); i++)
			{
				...
				Transaction trans = expected[i];
				
				assertEquals(trans.getPayee(), transaction.getPayee());
				assertEquals(trans.getMemo(), transaction.getMemo());
				assertEquals(trans.getPostedDate(), transaction.getPostedDate());
				assertEquals(trans.getTransferAmount(), transaction.getTransferAmount());
				assertEquals(trans.getDepositAccount().getName(), transaction.getDepositAccount().getName());
				assertEquals(trans.getWithdrawalAccount().getName(), transaction.getWithdrawalAccount().getName());
			}
