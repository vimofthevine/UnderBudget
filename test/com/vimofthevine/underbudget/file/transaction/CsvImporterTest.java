...
/**
 * Unit test case for the CsvImporter class
 * 
...
public class CsvImporterTest {
	...
	public void setUp()
	{
		progress = new TaskProgress("CSV import");
		...
		importer = new CsvImporter(currencyFactory);
	}
	...
	{		
		try
        {
			Transaction[] expected = new Transaction[] {
				...
				new ImportedTransaction(new SimpleDate(7, 11, 2011), "Mr. Mechanic",
					"", currencyFactory.newCurrencyInstance("402.44"),
					new Account(""), new Account("")),
				new ImportedTransaction(new SimpleDate(28, 6, 2010), "Vendor",
					"", currencyFactory.newCurrencyInstance("150.00"),
					new Account(""), new Account("")),
				new ImportedTransaction(new SimpleDate(28, 6, 2010), "Vendor",
					"", currencyFactory.newCurrencyInstance("14.68"),
					new Account(""), new Account("")),
				new ImportedTransaction(new SimpleDate(6, 1, 2012), "Early",
					"", currencyFactory.newCurrencyInstance("5.25"),
					new Account(""), new Account("")),
				new ImportedTransaction(new SimpleDate(7, 1, 2012), "On Time",
					"", currencyFactory.newCurrencyInstance("0.00"),
					new Account(""), new Account("")),
				new ImportedTransaction(new SimpleDate(8, 1, 2012), "Late",
					"", currencyFactory.newCurrencyInstance("5.25"),
					new Account(""), new Account("")),
			};
	...
	public void testParseDateFiltered()
	{
		...
		{
			stream = getClass().getResourceAsStream("mint.txt");
			List<Transaction> transactions = importer.read(stream,
				new MonthlyBudgetingPeriod(11, 2011), progress);
