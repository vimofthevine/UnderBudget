...
//------------------------------------------------------------------------------
const QMap<QString,QString>& currencies()
{
...
{
	// Since this list doesn't change during runtime, cache
	// the key list
	static QStringList codes(currencies().keys());
	return codes;
}
...
{
	return currencies().value(iso4217, QString('?'));
}
