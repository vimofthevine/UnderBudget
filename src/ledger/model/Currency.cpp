...
const QMap<QString, QString> & symbols() {
    ...
    if (codesToSymbols.size() == 0) {
        codesToSymbols.insert(
            "ALL", QString("%1%2%3").arg(QChar(76)).arg(QChar(101)).arg(QChar(107))); // Albania Lek
        ...
        codesToSymbols.insert("AZN", QString("%1%2%3")
                                         .arg(QChar(1084))
                                         .arg(QChar(1072))
                                         .arg(QChar(1085))); // Azerbaijan New Manat

        codesToSymbols.insert("BSD", QChar(36)); // Bahamas Dollar
        codesToSymbols.insert("BBD", QChar(36)); // Barbados Dollar
        codesToSymbols.insert("BYR",
                              QString("%1%2").arg(QChar(112)).arg(QChar(46))); // Belarus Ruble
        codesToSymbols.insert(
            "BZD", QString("%1%2%3").arg(QChar(66)).arg(QChar(90)).arg(QChar(36))); // Belize Dollar
        codesToSymbols.insert("BMD", QChar(36)); // Bermuda Dollar
        codesToSymbols.insert("BOB",
                              QString("%1%2").arg(QChar(36)).arg(QChar(98))); // Bolivia Boliviano
        codesToSymbols.insert("BAM", QString("%1%2").arg(QChar(75)).arg(
                                         QChar(77))); // Bosnia & Herzegovina Convertible Marka
        codesToSymbols.insert("BWP", QChar(80));      // Botswana Pula
        codesToSymbols.insert("BGN",
                              QString("%1%2").arg(QChar(1083)).arg(QChar(1074)));    // Bulgaria Lev
        ...
        codesToSymbols.insert("HRK",
                              QString("%1%2").arg(QChar(107)).arg(QChar(110))); // Croatia Kuna
        codesToSymbols.insert("CUP", QChar(8369));                              // Cuba Peso
        codesToSymbols.insert(
            "CZK", QString("%1%2").arg(QChar(75)).arg(QChar(269))); // Czech Republic Koruna

        codesToSymbols.insert("DKK",
                              QString("%1%2").arg(QChar(107)).arg(QChar(114))); // Denmark Krone
        codesToSymbols.insert("DOP", QString("%1%2%3").arg(QChar(82)).arg(QChar(68)).arg(
                                         QChar(36))); // Dominical Republic Peso
        ...
        codesToSymbols.insert("EEK",
                              QString("%1%2").arg(QChar(107)).arg(QChar(114))); // Estonia Kroon
        ...
        codesToSymbols.insert("HUF",
                              QString("%1%2").arg(QChar(70)).arg(QChar(116))); // Hungary Forint

        codesToSymbols.insert("ISK",
                              QString("%1%2").arg(QChar(107)).arg(QChar(114))); // Iceland Krona
        codesToSymbols.insert("INR", QChar(8425));                              // India Rupee
        codesToSymbols.insert("IDR",
                              QString("%1%2").arg(QChar(82)).arg(QChar(112))); // Indonesia Rupiah
        ...
        codesToSymbols.insert("JMD",
                              QString("%1%2").arg(QChar(74)).arg(QChar(36))); // Jamaican Dollar
        codesToSymbols.insert("JPY", QChar(165));                             // Japan Yen
        codesToSymbols.insert("JEP", QChar(163));                             // Jersey Pound

        codesToSymbols.insert(
            "KZT", QString("%1%2").arg(QChar(1083)).arg(QChar(1074))); // Kazakhstan Tenge
        codesToSymbols.insert("KPW", QChar(8361));                     // Korea (North) Won
        codesToSymbols.insert("KRW", QChar(8361));                     // Korea (South) Won
        codesToSymbols.insert("KGS",
                              QString("%1%2").arg(QChar(1083)).arg(QChar(1074))); // Kyrgyzstan Som
        ...
        codesToSymbols.insert("LTL",
                              QString("%1%2").arg(QChar(76)).arg(QChar(116))); // Lithuania Litas

        codesToSymbols.insert("MKD", QString("%1%2%3")
                                         .arg(QChar(1076))
                                         .arg(QChar(1077))
                                         .arg(QChar(1085))); // Macedonia Denar
        codesToSymbols.insert("MYR",
                              QString("%1%2").arg(QChar(82)).arg(QChar(77))); // Malaysia Ringgit
        ...
        codesToSymbols.insert("MZN",
                              QString("%1%2").arg(QChar(77)).arg(QChar(84))); // Mozambique Metical
        ...
        codesToSymbols.insert("NIO",
                              QString("%1%2").arg(QChar(67)).arg(QChar(36))); // Nicaragua Cordoba
        codesToSymbols.insert("NGN", QChar(8358));                            // Nigeria Naira
        codesToSymbols.insert("NOK",
                              QString("%1%2").arg(QChar(107)).arg(QChar(114))); // Norway Krone

        codesToSymbols.insert("OMR", QChar(65020)); // Oman Rial

        codesToSymbols.insert("PKR", QChar(8360)); // Pakistan Rupee
        codesToSymbols.insert(
            "PAB", QString("%1%2%3").arg(QChar(66)).arg(QChar(47)).arg(QChar(46))); // Panama Balboa
        codesToSymbols.insert("PYG",
                              QString("%1%2").arg(QChar(71)).arg(QChar(115))); // Paraguay Guarani
        codesToSymbols.insert("PEN", QString("%1%2%3").arg(QChar(83)).arg(QChar(47)).arg(
                                         QChar(46))); // Peru Nuevo Sol
        codesToSymbols.insert("PHP", QChar(8369));    // Philippines Peso
        codesToSymbols.insert("PLN",
                              QString("%1%2").arg(QChar(122)).arg(QChar(322))); // Poland Zloty

        codesToSymbols.insert("QAR", QChar(65020)); // Qatar Riyal

        codesToSymbols.insert(
            "RON",
            QString("%1%2%3").arg(QChar(108)).arg(QChar(101)).arg(QChar(105))); // Romania New Leu
        codesToSymbols.insert(
            "RUB",
            QString("%1%2%3").arg(QChar(1088)).arg(QChar(1091)).arg(QChar(1073))); // Russia Ruble

        codesToSymbols.insert("SHP", QChar(163));   // Saint Helena Pound
        codesToSymbols.insert("SAR", QChar(65020)); // Saudi Arabia Riyal
        codesToSymbols.insert("RSD", QString("%1%2%3%4")
                                         .arg(QChar(1044))
                                         .arg(QChar(1080))
                                         .arg(QChar(1085))
                                         .arg(QChar(46))); // Serbia Dinar
        ...
        codesToSymbols.insert("SEK",
                              QString("%1%2").arg(QChar(107)).arg(QChar(114))); // Sweden Krona
        codesToSymbols.insert("CHF", QString("%1%2%3").arg(QChar(67)).arg(QChar(72)).arg(
                                         QChar(70))); // Switzerland Franc
        codesToSymbols.insert("SRD", QChar(36));      // Suriname Dollar
        codesToSymbols.insert("SYP", QChar(163));     // Syria Pound

        codesToSymbols.insert("TWD", QString("%1%2%3").arg(QChar(78)).arg(QChar(84)).arg(
                                         QChar(36))); // Taiwan New Dollar
        codesToSymbols.insert("THB", QChar(3647));    // Thailand Baht
        codesToSymbols.insert("TTD", QString("%1%2%3").arg(QChar(84)).arg(QChar(84)).arg(
                                         QChar(36))); // Trinidad and Tobago Dollar
        ...
        codesToSymbols.insert("UZS",
                              QString("%1%2").arg(QChar(1083)).arg(QChar(1074))); // Uzbekistan Som

        codesToSymbols.insert("VEF",
                              QString("%1%2").arg(QChar(66)).arg(QChar(115))); // Venezuela Bolivar
        codesToSymbols.insert("VND", QChar(8363));                             // Viet Nam Dong

        codesToSymbols.insert("YER", QChar(65020)); // Yemen Rial

        codesToSymbols.insert("ZWD",
                              QString("%1%2").arg(QChar(90)).arg(QChar(36))); // Zimbabwe Dollar
    }
